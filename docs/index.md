...
- :material-chip: __Backend adapters__
    ...
    Three adapters ship: **DeepAgentsMLX** for on-device LFM2.5 inference via Apple MLX; **DeepAgentsOpenAI** for OpenAI, OpenRouter, Azure, and any OpenAI-compatible endpoint; **DeepAgentsAnthropic** for the Anthropic Messages API and AWS Bedrock with SigV4 or a bearer token. The core framework carries zero backend dependencies.
...
| `DeepAgentsAnthropic` | Anthropic Messages API + AWS Bedrock (SigV4 or bearer token) | CryptoKit |
