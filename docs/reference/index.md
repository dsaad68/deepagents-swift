...
| `BedrockChatModel` | `ChatModel` for AWS Bedrock (Anthropic models via SigV4 or a bearer token) |
| `BedrockAuth` | Bedrock auth method: `.sigV4(BedrockCredentials)` or `.bearerToken(String)`; `resolve(bearerToken:)` picks one |
