...
LFM2.5 models run with a 32k-token context window, except the 2.6B, which is run at the full 128k
its model card documents. Recommended sampling parameters are baked into each
`MlxModel.agentParameters` entry so you can pass them straight through.

| Model | Kind | approx. size | Notes |
|---|---|---|---|
| LFM2.5 350M | language | ~0.4 GB | Below the reliable tool-call floor; useful for classification/summarization tasks without tools |
| LFM2.5 1.2B Instruct | language | ~1.3 GB (8-bit) / ~2.5 GB (bf16) | General-purpose; reliable tool calling |
| LFM2.5 1.2B Thinking | language | ~1.3 GB (8-bit) / ~2.4 GB (bf16) | Reasoning mode; uses `top_p 0.1` sampling; higher `maxTokens` (8192) to fit `<think>` blocks |
| LFM2.5 2.6B | language | ~1.6 GB (MXFP4) / ~2.8 GB (MXFP8) / ~2.9 GB (8-bit) / ~5.4 GB (bf16) | General-purpose 2.69B; 128k context; `repetition_penalty 1.1` rather than the family's 1.05. Reasons in `<think>` blocks, so `maxTokens` is 8192 and `startsInReasoning` is set. Four precisions in one repo - see below |
| LFM2.5 8B-A1B | language | ~9 GB | Mixture-of-experts; strongest tool-calling in the family |
| LFM2.5 VL 450M | vision | ~0.6 GB (8-bit) / ~1.0 GB (bf16) | Vision-language; `supportsVision: true` |
| LFM2.5 VL 1.6B | vision | ~2.1 GB | Vision-language; higher accuracy than 450M |

!!! note "Precision subfolders"
    Most catalog ids are Hugging Face repo ids. LFM2.5 2.6B is the exception: LiquidAI publishes
    every precision as a subfolder of one repo, so its ids carry a third component naming the
    precision - `LiquidAI/LFM2.5-2.6B-MLX/mxfp4`, `…/mxfp8`, `…/8bit`, `…/bf16`.

    The id stays the unique selection key; `MlxModelID` splits it, and `MlxModelLoader` scopes the
    download to that subfolder and loads from `snapshots/<hash>/<precision>/`. Each precision
    downloads, reports downloaded state, and deletes independently even though all four share one
    cache directory. `MXFP4` and `MXFP8` are MXFP microscaling formats; MXFP4 keeps
    `model.embed_tokens` at MXFP8 via a per-layer quantization override.
