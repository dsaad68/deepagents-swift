...
family (350 M to 8 B-A1B, including a 2.6 B available in four precisions) covers most agentic
tasks; VL variants handle vision. Models are pulled once from Hugging Face and cached locally.
...
to `api.anthropic.com`; `BedrockChatModel` talks to AWS Bedrock with SigV4 request signing or a
bearer-token API key, and cross-region inference profile IDs.
