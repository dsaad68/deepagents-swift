...
=== "Anthropic (cloud)"
    ...
    For AWS Bedrock, use `BedrockChatModel` with a `BedrockCredentials` value (or `BedrockCredentials.fromEnvironment()`) instead. The model id can be a cross-region inference-profile id (e.g. `us.anthropic.claude-...`).
