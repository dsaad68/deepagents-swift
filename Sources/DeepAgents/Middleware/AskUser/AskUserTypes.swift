...
enum AskUser {
    ...
    /// (`{value}`) or bare strings, a missing `type` inferred from whether choices are present,
    /// and the whole array delivered as a JSON *string* (a tool-call parser without the schema in
    /// hand leaves nested values untyped).
    static func parseQuestions(_ value: AgentJSON?) -> [AskUserQuestion] {
        ...
        case .string(let text):
            guard let data = text.data(using: .utf8),
                  let decoded = try? JSONDecoder().decode(AgentJSON.self, from: data)
            else { return [] }
            switch decoded {
            case .array, .object: return parseQuestions(decoded)
            default: return []
            }
        default: return []
        }
