...
public struct MlxModel: Identifiable, Sendable, Hashable {
    ...
    /// repo it gains a third component naming the subfolder -
    /// "LiquidAI/LFM2.5-2.6B-MLX/mxfp4" - so each precision stays separately selectable,
    /// downloadable, and deletable. Use ``repoId`` / ``subfolder`` to take it apart; never hand this
    /// value to the Hub directly.
    ...
    /// additionally wants top-p 0.1; all take Liquid's repetition penalty 1.05 except the 2.6B,
    /// whose card asks for 1.1. Qwen3.6 cards want
    ...
    public var agentParameters: GenerateParameters {
        ...
        if isTwoPointSixB {
            ...
            // a *higher* repetition penalty (1.1, not the family's 1.05) - so this can't fall
            // through to the default below. It also reasons before answering (see
            // ``startsInReasoning``), so it gets the thinking models' larger token budget.
            return .init(maxTokens: 8192, temperature: 0.1, topK: 50, repetitionPenalty: 1.1)
        }
    ...
    /// True for the LFM2.5-2.6B rows (all four precision subfolders of one repo) - they share a
    ...
    /// template ends at `assistant\n` and generates the whole `<think>…</think>` itself. Getting
    /// this wrong leaks the entire reasoning pass into the visible answer, trailed by a bare
    /// `</think>`. (The qwen3_5 codec hard-codes the same behaviour - Ornith's template prefills it
    ...
    public static let catalog: [MlxModel] = [
        ...
        // LFM2.5-2.6B is the first catalog conversion that ships *every* precision as a subfolder
        // of one repo (`bf16/ 8bit/ mxfp8/ mxfp4/`, plus 4/5/6-bit and nvfp4 we don't list) rather
        // than one repo per precision. The id carries the subfolder as a third path component so it
        // stays the unique selection key - see ``MlxModelID``; ``MlxModelLoader`` splits it for the
        // calls that talk to the Hub and loads from `snapshots/<hash>/<subfolder>/`. MXFP8/MXFP4 are
        // MXFP microscaling formats (mlx-swift's `QuantizationMode`); MXFP4 keeps
        // `model.embed_tokens` at MXFP8, which mlx-swift-lm decodes as a per-layer override.
        MlxModel(id: "LiquidAI/LFM2.5-2.6B-MLX/mxfp4",
                 displayName: "LFM2.5 2.6B", detail: "Instruct · MXFP4", kind: .language, approxGB: 1.6),
        MlxModel(id: "LiquidAI/LFM2.5-2.6B-MLX/mxfp8",
                 displayName: "LFM2.5 2.6B", detail: "Instruct · MXFP8", kind: .language, approxGB: 2.8),
        MlxModel(id: "LiquidAI/LFM2.5-2.6B-MLX/8bit",
                 displayName: "LFM2.5 2.6B", detail: "Instruct · 8-bit", kind: .language, approxGB: 2.9),
        MlxModel(id: "LiquidAI/LFM2.5-2.6B-MLX/bf16",
                 displayName: "LFM2.5 2.6B", detail: "Instruct · bf16 · large", kind: .language, approxGB: 5.4),
