...
public struct MlxChatModel: ChatModel {
    ...
    /// Whether this model's chat template prefills the opening `<think>` into the generation prompt,
    /// so its stream starts *inside* reasoning; set from ``MlxModel/startsInReasoning``.
    public var startsInReasoning = false
    ...
    public init(
        ...
        self.startsInReasoning = startsInReasoning
...
public final class RebuildTurnSession: ModelTurnSession {
    ...
    /// Whether the model's stream starts inside a `<think>` block its template already opened.
    private let startsInReasoning: Bool
    ...
    public init(
        ...
        codecFamily: MlxCodecFamily = .lfm2, startsInReasoning: Bool = false,
        prefixCache: PrefixCacheSlot = PrefixCacheSlot(), modelID: String? = nil
    ) {
        self.container = container
        self.supportsVision = supportsVision
        self.generateParameters = generateParameters
    ...
    public func nextTurn(
        ...
        return try await container.perform { context in
            ...
            let decoder = Self.makeDecoder(family: family, startsInReasoning: startsInReasoning)
    ...
    /// qwen3_5 codec always starts in reasoning (Ornith's template prefills `<think>` too) and the
    /// gemma4 thought channel is delimited differently.
    private static func makeDecoder(
        family: MlxCodecFamily, startsInReasoning: Bool
    ) -> any TurnDecoder<String> {
        switch family {
        case .lfm2: return LFM2MessageCodec().makeDecoder(startInThink: startsInReasoning)
