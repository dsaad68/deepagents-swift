...
public struct LFM2MessageCodec: MessageCodec {
    ...
    /// `startInThink` is for LFM2 models whose chat template prefills the opening `<think>` into the
    /// generation prompt (LFM2.5-2.6B): their stream begins inside reasoning and only ever emits the
    /// closing tag, so without this the whole reasoning pass leaks into the answer followed by a
    /// bare `</think>`. Most LFM2 models generate the full `<think>…</think>` themselves, which is
    /// what the protocol's no-argument ``makeDecoder()`` gives you.
    public func makeDecoder(startInThink: Bool) -> any TurnDecoder<String> {
        LFM2Decoder(startInThink: startInThink)
    }
...
public final class LFM2Decoder: TurnDecoder {
    ...
    /// `startInThink`: the model's chat template already emitted the opening `<think>`, so the
    /// stream starts inside reasoning - see ``LFM2MessageCodec/makeDecoder(startInThink:)``.
    public init(startInThink: Bool = false) {
        thinkSplitter = ThinkStream(startInThink: startInThink)
    }
