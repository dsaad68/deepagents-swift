...
struct CrossCodecTests {
    ...
    @Test func qwen35ProducedTurnReEncodesForOpenAIDroppingReasoning() {
        // The qwen3_5 decoder collects answer + reasoning from chunks and the tool call from a
        // library `Generation.toolCall` event, then yields the same canonical message shape.
        let decoder = Qwen35Decoder()
        // Ornith prefills the opening `<think>`, so the generated stream starts inside reasoning.
        _ = decoder.ingest("plan</think>I'll call echo.")
        decoder.ingestToolCall(ToolCall(function: .init(name: "echo", arguments: ["text": .string("x")])))
        let aiTurn = decoder.finish().message
        #expect(aiTurn.reasoning == "plan")
        #expect(aiTurn.toolCalls.first?.name == "echo")
        let callID = aiTurn.toolCalls.first?.id
