...
struct AskUserMiddlewareTests {
    ...
    /// without the schema in hand (the qwen3_5 XML path before `generateTask` got `tools:`)
    /// leaves nested values untyped, and this call used to fail with "requires at least one
    /// question" despite carrying a perfectly valid question list.
    @Test func questionsAsJSONStringParse() async throws {
        let script = AnswerScript(.answered(["Dan"]))
        let tool = AskUserTool(handler: script.handler)
        let json = #"[{"question": "What's your name?", "type": "text", "required": false}]"#
        let output = try await tool.execute(["questions": .string(json)], ToolContext())
        #expect(output.content == "Q: What's your name?\nA: Dan")
        let request = try #require(await script.requests.first)
        #expect(request.questions.map(\.required) == [false])
        // Garbage strings still fail validation instead of crashing or asking nothing.
        let bad = try await tool.execute(["questions": .string("not json")], ToolContext())
        #expect(bad.content.contains("at least one question"))
    }
